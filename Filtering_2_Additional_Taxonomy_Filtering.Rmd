...
gm3_stringent_2025_clean <- gm3_merged_with_taxa %>%
 ...
 gm3_stringent_2025_clean2 <- gm3_stringent_2025_clean %>% 
...
save(gm3_stringent_2025_clean2, file = "gm3_stringent_2025_clean2") #saving the file for quick reloading later on

load("gm3_stringent_2025_clean2")

#time took = 5 mins, ran fine the first time
```

#jUNE 2025 - redoing the above filter to remove any reads that have species.y as NA. Whats happening is that identical reads are only assigned taxonomy to one of the reads, while the other ones is blank for superkingdom to species.y but does have a species.x name. Then when the reads get randomly chosen at the weighted filtering step (because they are exactly the same) sometimes the read that is retained is the one without any higher taxonomy info which means that if youre looking for species in your samples, searching the species.x or subject tax ID's will work, but the species.y column wont. It also means any phyla or genus level figures etc are msissing data
