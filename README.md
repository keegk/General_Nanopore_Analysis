...
**Basecalled, compressed fastq files using fast accuracy Guppy v6.0.1:** /mnt/shared/projects/jhi/bioss/kkeegan_onttestdata/Seal_MinION_analysis/Basecalling_and_fastq/Rebasecalled_jan2023.tar.gz. Within each numbered directory, you will find a  pass subdirectory with a fastq read in it. /mnt/shared/projects/jhi/bioss/kkeegan_onttestdata/Seal_MinION_analysis/Basecalling_and_fastq/Rebasecalled_jan2023 was used as the input path for the bash script cat_fastq.sh, where all the fastq files with 'pass' in their pathname were concatenated together, leaving the failed fastq reads untouched. 

Additional check: The final concat.fastq file has 1,051,609 fastq files and I checked to see if any failed fastq files made it into concat.fastq by searching concat.fastq for a handful of failed fastq files (finding some failed fastq files in /mnt/shared/projects/jhi/bioss/kkeegan_onttestdata/Seal_MinION_analysis/Basecalling_and_fastq/Rebasecalled_jan2023 and searching their read ID in concat.fastq with., e.g  **more concat.fastq | grep @18ef8eec-d3d8-498b-acba-cadb5f57a2
b7| wc -l**). Three failed fastq files tested all returned 0 lines so I know the failed fastq files arent part of concat.fastq. I also confirmed the pass fastq files were going into concat.fastq by checking a randomly selected pass fastq file from Rebasecalled_jan2023 and searching its read ID. The line count returned 1 , so I know it is in the concat.fastq file.

**Post-basecalled data including raw fasta files, raw blast files, concatenated cleaned fasta and concatenated blast files**
1) The cleaned fasta file that was used as input for the seal BLAST run is: /mnt/shared/projects/jhi/bioss/kkeegan_onttestdata/Seal_MinION_analysis/Dataset_post_basecalling/fasta_split/cleaned_fasta.fasta. It was first split into many files each containing 5,000 reads (/mnt/shared/projects/jhi/bioss/kkeegan_onttestdata/Seal_MinION_analysis/Dataset_post_basecalling/fasta_split) and each fasta file was run through the blast7.sh script (BLAST database used is NCBI  database on cluster).
