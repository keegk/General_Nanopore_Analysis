...
Visualising the blastn AMR gene results 

```{r}

#install.packages("plotly")
library(plotly) #for sunburst charts

sunburst_df_large <- gm_blastn_CARD_2025_taxa_mge_cleaned %>%
  group_by(Resistance.Mechanism, model_type, species.x, Major.mobileOG.Category) %>%
  filter(Resistance.Mechanism == "antibiotic target alteration") %>%
  summarise(count = n()) %>%
  mutate(species.x = replace_na(species.x, "Unassigned")) %>%
  mutate(Major.mobileOG.Category = replace_na(Major.mobileOG.Category, "No Mobile Genetic Element")) %>%
  ungroup()

sunburst_df_small <- gm_blastn_CARD_2025_taxa_mge_cleaned %>%
  group_by(Resistance.Mechanism, model_type, species.x, Major.mobileOG.Category) %>%
 # mutate(rename(Resistance.Mechanism("reduced permeability;resistance by absence" == "reduced permeability to antibiotic; resistance by absence"))) %>%
  filter(!Resistance.Mechanism == "antibiotic target alteration") %>%
  summarise(count = n()) %>%
  mutate(species.x = replace_na(species.x, "Unassigned")) %>%
  mutate(Major.mobileOG.Category = replace_na(Major.mobileOG.Category, "No Mobile Genetic Element")) %>%
...
sunburst_df_ph <- gm_blastn_AMR_with_ph %>%
  group_by(Resistance.Mechanism, species.x, Major.mobileOG.Category) %>%
 # mutate(rename(Resistance.Mechanism("reduced permeability;resistance by absence" == "reduced permeability to antibiotic; resistance by absence"))) %>%
  #filter(model_type == "protein homolog model") %>%
  summarise(count = n()) %>%
  mutate(species.x = replace_na(species.x, "Unassigned")) %>%
...
sunburst_df_ph_mge <- gm_blastn_AMR_with_mge_ph %>%
  group_by(Resistance.Mechanism, species.x, Major.mobileOG.Category) %>%
 # mutate(rename(Resistance.Mechanism("reduced permeability;resistance by absence" == "reduced permeability to antibiotic; resistance by absence"))) %>%
  #filter(model_type == "protein homolog model") %>%
  summarise(count = n()) %>%
...
#This function turns a data frame into a hierarchical data structure, TAKEN FROM https://rpubs.com/DragonflyStats/Sunburst-Plots-With-Plotly
...
sunburstDF_large <- as.sunburstDF(sunburst_df_large, value_column = "count", add_root = TRUE)
sunburstDF_small <- as.sunburstDF(sunburst_df_small, value_column = "count", add_root = TRUE)
sunburst_df_ph <- as.sunburstDF(sunburst_df_ph, value_column = "count", add_root = TRUE)
sunburstDF_ph_mge <- as.sunburstDF(sunburst_df_ph_mge, value_column = "count", add_root = TRUE)
#head(sunburstDF)

#fig <- plot_ly(sunburstDF, ids = ~ids, labels = ~labels, parents = ~parents, type = 'sunburst')

#fig
```

```{r} 
#just antibiotic inactivation
fig <- plot_ly()

fig<- fig %>% add_trace(
  type='sunburst',
  ids=sunburstDF_large$ids,
  labels=sunburstDF_large$labels,
  parents=sunburstDF_large$parents,
 # values = sunburstDF_large$values,
  insidetextorientation='auto'
 #makes the text show in a circle around the plot
 ) %>%
   
layout(
  uniformtext = list(minsize = 8, mode = 'show')#,  # Adjust minsize to your desired font size
 # title = "Breakdown of Predominat Resistance Mechanism Detected: Antibiotic Target Alteration" # used this resource to generate the font size code https://plotly.com/r/text-and-annotations/, 
...
```{r} 
#all the other resistance mechanisms
fig <- plot_ly()

fig<- fig %>% add_trace(
  type='sunburst',
  ids=sunburst_df_ph$ids,
  labels=sunburst_df_ph$labels,
  parents=sunburst_df_ph$parents,
 # values = sunburstDF_large$values,
  insidetextorientation='auto'
 #makes the text show in a circle around the plot
 ) %>%
...
layout(
  uniformtext = list(minsize = 6, mode = 'hide'),  # Adjust minsize to your desired font size
 # title = "Breakdown of other Antibiotic Resistance Mechanisms in the Grey Seal Dataset ", # used this resource to generate the font size code https://plotly.com/r/text-and-annotations/, 
  
annotations = list(
 list(
 text = "reduced permeability to antibiotic; resistance by absence",
x = 0.55, # Adjust x and y coordinates to position the annotation correctly
 y = 0.44,
 font = list(
 family = "Arial",
 size = 8)
...
showarrow = FALSE,
arrowhead = 2,
 ax = 20, # Adjust arrow x position (length)
 ay = -28 # Adjust arrow y position (length)
...
sunburstDF_ph_mge - PH and MGE RESULTS

```{r} 
#protein homolog hits only - resistance mechanisms
library(plotly) 
fig <- plot_ly()

fig<- fig %>% add_trace(
  type='sunburst',
  ids=sunburstDF_ph_mge$ids,
  labels=sunburstDF_ph_mge$labels,
  parents=sunburstDF_ph_mge$parents,
 # values = sunburstDF_large$values,
  insidetextorientation='auto'
 #makes the text show in a circle around the plot
 ) %>%
